...
class DetectionModel(BaseModel):
    def __init__(self, config:str, in_channels:int, n_classes=None):
        ...
        # (40,40,1024,w,1+r) -> (40,40,512,w)
        self.c2f12 = C2f(1024, 512, n=max(round(3*depth), 1))

        # (40,40,512,w) -> (80,80,256,w)
        self.upsample13 = nn.Upsample(size=None, scale_factor=2, mode='nearest')

        self.c2f15 = C2f(512, 256, n=max(round(3*depth), 1))

        # (80,80,256,w) -> (40,40,256,w)
        self.conv16 = Conv(256, 256, kernel_size=3, stride=2)

        self.c2f18 = C2f(512, 512, n=max(round(3*depth), 1))
